...
function validateDefinition(definition) {
  ...
  nodes.forEach((node) => {
    ...
    if (outgoing.length > 1) {
      errors.push(
        `node ${nodeId} has ${outgoing.length} outgoing edges; only decide nodes can branch`,
      );
    }
...
async function main() {
  ...
  if (workflowId) {
    ...
    if (!definition || typeof definition !== "object") {
      printJson(
        err("Workflow definition missing in response", response.raw, false, response.status),
      );
      return 2;
    }

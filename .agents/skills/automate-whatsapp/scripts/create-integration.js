...
function usage() {
  return ok({
    usage:
      "node scripts/create-integration.js --action-id <id> --app-slug <slug> --account-id <id> [--configured-props <json>] [--name <text>] [--app-name <text>] [--variable-definitions <json>] [--dynamic-props-id <id>]",
    notes: [
...
async function resolveAccountId(config, appSlug, accountId) {
  ...
  if (!response.ok) {
    throw new Error(
      "Unable to resolve account id. Use list-accounts and pass pipedream_account_id.",
    );
  }
...
async function main() {
  ...
  try {
    ...
    variableDefinitions = parseJson(
      getFlag(parsed.flags, "variable-definitions"),

...
async function main() {
  ...
  const expectedLockVersion =
    getNumberFlag(parsed.flags, "expected-lock-version") ??
  ...
  if (!current.ok) {
    printJson(
      err("Failed to fetch workflow metadata for lock check", current.raw, false, current.status),
    );
    return 2;
  }
  ...
  if (currentLock !== expectedLockVersion) {
    printJson(
      err("Conflict: workflow was modified. Refetch and retry.", {
